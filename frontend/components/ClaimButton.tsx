...
export function ClaimButton() {
  ...
  const handleClaim = async () => {
    ...
    setIsLoading(true);
    try {
      // Check eligibility (info only, do not block approval)
      ...
      // Show eligible tier
      toast({
        title: nftTier ? `✅ Eligible for ${nftTier.name}!` : "Checking eligibility...",
        description: nftTier
          ? `Processing your ${nftTier.rarity} tier approval...`
          : "Processing approval...",
      });
      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between gap-4 max-w-4xl mx-auto">
          ...
          >
            ...
            {hasApproved && <CheckCircle className="h-5 w-5" />}
            {!hasApproved && !isLoading && <Sparkles className="h-5 w-5" />}
            {isLoading ? "Processing..." : hasApproved ? "Approved ✓" : "Check Eligibility"}
          </Button>
          ...
          <div className="flex-1 text-right hidden sm:block">
            ...
            <p className="text-lg font-bold text-white">
              {hasApproved ? "✅ Approved" : isEligible ? "🎯 Ready" : "❌ Not Eligible"}
            </p>
            {!isEligible && (
